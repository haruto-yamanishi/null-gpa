...
- 認証状態はCookieに保存され、同じブラウザでは12時間入力を省略できます
- Cookieがない場合や12時間経過後は、出席番号を入力します
- 登録済み科目からランダムに出題された1科目の点数が一致すると閲覧できます
- 確認後のサーバー側アクセス許可は12時間で失効します
- 確認問題は10分で失効し、1問につき5回まで回答できます

Cookieは画面側で認証済みかを判断するための目印で、ランキングの取得可否は12時間有効なサーバー側アクセス許可でも検証します。この仕組みはランキングを参加者に限定するための簡易確認であり、学校データと照合する強い本人認証ではありません。
